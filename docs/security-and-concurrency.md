...
- 裁剪结果必须为可完整解码的 JPEG。
- 活动封面尺寸必须为 `1200×500`，头像必须为 `512×512`。
- 新文件写入后如果数据库保存失败，会补偿删除新文件。
- 替换或移除图片时，只在数据库成功后删除旧文件。
- 删除只接受对应上传目录中的 UUID 文件名；外部 URL、默认图和路径穿越输入不会触发文件删除。
